...
{
    public void Dispose() => ApiAlerts.Reset();

    private static ApiAlertsClient NewClient(string apiKey, HttpClient httpClient) =>
        new(apiKey, debug: false, NullLogger.Instance, httpClient);
    ...
    {
        var client = NewClient("", MockHttp.Success());
    ...
    {
        var client = NewClient("key", MockHttp.Success());
    ...
    {
        var client = NewClient("key", MockHttp.Success("W", "C"));
        var result = await client.SendAsync(new Event { Message = "test" });
    ...
    {
        var client = NewClient("key", MockHttp.Success(warnings: new[] { "deprecated" }));
        var result = await client.SendAsync(new Event { Message = "test" });
        Assert.True(result.Success);
    ...
    {
        var client = NewClient("key", MockHttp.Client(HttpStatusCode.BadRequest));
        var result = await client.SendAsync(new Event { Message = "test" });
        Assert.False(result.Success);
    ...
    {
        var client = NewClient("key", MockHttp.Client(HttpStatusCode.Unauthorized));
        var result = await client.SendAsync(new Event { Message = "test" });
        Assert.False(result.Success);
    ...
    {
        var client = NewClient("key", MockHttp.Client(HttpStatusCode.Forbidden));
        var result = await client.SendAsync(new Event { Message = "test" });
        Assert.False(result.Success);
    ...
    {
        var client = NewClient("key", MockHttp.Client(HttpStatusCode.TooManyRequests));
        var result = await client.SendAsync(new Event { Message = "test" });
        Assert.False(result.Success);
    ...
    {
        var client = NewClient("key", MockHttp.Client(HttpStatusCode.InternalServerError));
        var result = await client.SendAsync(new Event { Message = "test" });
        Assert.False(result.Success);
    ...
    {
        var client = NewClient("key", MockHttp.Client(HttpStatusCode.OK, "not json"));
        var result = await client.SendAsync(new Event { Message = "test" });
        Assert.False(result.Success);
    ...
    public async Task SendAsync_Sends_AuthorizationHeader()
    {
        ...
        var client = NewClient("my-api-key", new HttpClient(handler));
    ...
    public async Task SendAsync_Sends_IntegrationHeaders()
    {
        var handler = new MockHttpMessageHandler(HttpStatusCode.OK,
            """{"workspace":"W","channel":"C","warnings":[]}""");
        var client = NewClient("key", new HttpClient(handler));
        await client.SendAsync(new Event { Message = "test" });
        Assert.Equal("csharp", handler.LastRequest!.Headers.GetValues("X-Integration").First());
        Assert.Matches(@"^\d+\.\d+\.\d+", handler.LastRequest!.Headers.GetValues("X-Version").First());
    }
    ...
    public async Task SendWithKey_Uses_ProvidedApiKey()
    {
        var handler = new MockHttpMessageHandler(HttpStatusCode.OK,
            """{"workspace":"W","channel":"C","warnings":[]}""");
        var client = NewClient("original-key", new HttpClient(handler));
        await client.SendAsync(new Event { Message = "test" }, apiKey: "override-key");
    ...
    [Fact]
    public void Send_DoesNotThrow_OnError()
    {
        var client = NewClient("key", MockHttp.Client(HttpStatusCode.Unauthorized));
        // Should complete without throwing
        client.Send(new Event { Message = "test" });
    }

    // ── Global singleton ──────────────────────────────────────────────────────

    [Fact]
    public async Task ApiAlerts_SendAsync_Returns_Failure_BeforeConfigure()
    {
        var result = await ApiAlerts.SendAsync(new Event { Message = "test" });
        Assert.False(result.Success);
        Assert.Equal("client not configured", result.Error);
    }

    [Fact]
    public async Task ApiAlerts_Configure_InitializesClient()
    {
        ApiAlerts.Configure("key", debug: false, logger: NullLogger.Instance, httpClient: MockHttp.Success("W", "C"));
        var result = await ApiAlerts.SendAsync(new Event { Message = "test" });
        Assert.True(result.Success);
        Assert.Equal("W", result.Workspace);
    }
    ...
    [Fact]
    public async Task ApiAlerts_Configure_IsIdempotent()
    {
        ApiAlerts.Configure("first-key", debug: false, logger: NullLogger.Instance, httpClient: MockHttp.Success());
    ...
    [Fact]
    public void ApiAlerts_Send_DoesNotThrow_BeforeConfigure()
    {
        // Should complete without throwing
        ApiAlerts.Send(new Event { Message = "test" });
    }
}
